    ...
    if (!forceMode) {
      for (let i = 0; i < checklistList.length; i++) {
        if (checklistList[i].fill === false)
          return checklistList[i].checklist_id === id
      }
    }
    return true
  }
    ...
    {checklistList && currentPatient ? <div className="container">
      <div className={"iq-card bg-primary col-sm-6 mx-auto text-center p-2 shadow border border-dark justify-content-center "}>
        <div className="card-body">
          ...
          <h4 className="card-title text-white m-0">{currentPatient.intervention_name}</h4>
      ...
      <div className={"iq-card iq-bg-secondary col-sm-6 mx-auto text-center p-2 shadow border border-dark justify-content-center "}>
        <div className="card-body m-0 p-2">
          <h4 className="card-title text-dark m-0 p-0">Note</h4>
          {!forceMode ? <h6 className="card-text text-dark m-0 p-0">Vous devez choisir la prochaine checklist non remplie. Si vous souhaitez quand même en choisir une autre, appuyer ici</h6>: null}
          {!forceMode ? <button className={"btn btn-warning m-0 mt-2 mb-1"} onClick={() => setForceMode(!forceMode)}>Autorise choix libre</button>: null}
          {forceMode ? <h6 className="card-text text-dark m-0 p-0">Vous pouvez choisir la checklist que vous souhaitez</h6>: null}
      ...
      <div className={"list-group text-center text-dark justify-content-center " + (list_length < 3 ? "list-group-horizontal":"")} >
        {checklistList.map( (checklist, index) => (
          <div className={"list-group-item shadow " + (list_length < 3 ? "col-sm-3 ":"col-sm-6 mx-auto list-group-item-rounded ") + (checklist.fill ? "iq-bg-success":"iq-bg-danger")}>
            ...
            <p>{checklist.name}</p>
            <button className=" btn btn-primary m-2" type="button" onClick={function (){if (check_if_fillable(checklist.checklist_id)) swapchecklist(checklist.checklist_id) }}>
            Remplir
            </button>
          </div>
        ))}
